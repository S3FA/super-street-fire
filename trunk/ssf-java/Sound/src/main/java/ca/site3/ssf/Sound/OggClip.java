...
public class OggClip {
	...
	public OggClip(String ref) throws IOException {
		try {
			init(new FileInputStream(ref));//Thread.currentThread().getContextClassLoader().getResourceAsStream(ref));
		} catch (IOException e) {
	...
	/**
	 * Loop the clip a set number of times
	 */
	public void loop(final int numberOfPlays) {
		stop();
		...
		player = new Thread() {
			public void run() {
				int playNumber = 0;
				while (player == Thread.currentThread() && playNumber <= numberOfPlays) {
					playNumber++;
